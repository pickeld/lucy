...
print("✅ Rich response processor imported", flush=True)
...
# Rich response post-processor
rich_processor = RichResponseProcessor()

# Data directories for media and events
DATA_DIR = Path(__file__).resolve().parent.parent / "data"
MEDIA_IMAGES_DIR = DATA_DIR / "images"
MEDIA_EVENTS_DIR = DATA_DIR / "events"
...
@app.route("/media/images/<path:filename>", methods=["GET"])
def serve_media_image(filename: str):
    """Serve a saved media image (WhatsApp photos, etc.).
    
    Images are persisted to data/images/ during webhook processing
    and referenced by media_path in Qdrant metadata.
    """
    from flask import send_from_directory
    ...
    return send_from_directory(
        str(MEDIA_IMAGES_DIR),
        filename,
        max_age=86400,  # Cache for 24h — images don't change
    )


@app.route("/media/events/<path:filename>", methods=["GET"])
def serve_media_event(filename: str):
    """Serve a generated ICS calendar event file for download.
    
    ICS files are generated by the RichResponseProcessor when the
    LLM creates calendar events in response to user requests.
    """
    from flask import send_from_directory
